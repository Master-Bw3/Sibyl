...
object SuggestionRenderer {
    ...
    private val suggestionGlyphPatternRenderer = PatternRenderer()
    private val suggestionGlyphCircleRenderer = CircleRenderer(true,true, 0)

    init {
        suggestionGlyphCircleRenderer.setColor(0.5f, 0.33f, 0.5f)
    }

    fun renderSuggestionGlyph(suggestion: Pattern) = { ctx: BraidDrawContext, transform: WidgetTransform ->
        suggestionGlyphPatternRenderer.render(
            PatternGlyph(suggestion),
            ctx.matrices,
            ctx.vertexConsumers,
            CircleWidget.RADIUS,
            CircleWidget.RADIUS,
...
//            val drawingSet: HashSet<Pattern.PatternEntry> = HashSet(Pattern.from(self.drawingPattern).entries())
...
//            //pattern
//            self._drawGlyph(
//                matrices, vertexConsumers, SpellPart(glyph), x, y, size, startingAngle, delta, alphaGetter, normal
//            )
//
